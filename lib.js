...
exports.readCurrent = async ({ currentTag, filter }) => {
 ...
 * }
 */
const parseInputCheck = (projectName, project, stageName, stage, name, input, config) => {
    ...
    Object.entries(projects).forEach(([projectName, project]) => {
        ...
        Object.entries(project.stages).forEach(([stageName, stage]) => {
            ...
            Object.entries(stage.https).forEach(([key, value]) => {
                const input = parseInputCheck(projectName, project, stageName, stage, key, value, config);
                if (config.inputTag && input.tags.indexOf(config.inputTag) < 0) return;
                const filterRexp = RegExp(config.filter, 'i');
                if (filterRexp && !filterRexp.test(input.name)) return;

                if (res.has(input.name))
                    throw new Error(`Duplicit check name on input: ${input.name}`);
...
exports.update = async (filename, config) => {
    ...
    const current = await exports.readCurrent({ currentTag, filter });
    ...
    const deletes = await Promise.all(Array.from(current.values()).map((check) => deleteExisting(check, config)));
